...
// History stuff
TUNE_PARAM(maxHistoryBonus, 2290, 1, 4096, 128, 0.002);
TUNE_PARAM(historyBonusQuadDepth, 8, 1, 32, 1, 0.002);
TUNE_PARAM(historyBonusLinDepth, 219, 1, 1024, 16, 0.002);
TUNE_PARAM(historyBonusOffset, -349, -1024, 1024, 52, 0.002);

TUNE_PARAM(maxHistoryMalus, 728, 1, 4096, 128, 0.002);
TUNE_PARAM(historyMalusQuadDepth, 5, 1, 32, 1, 0.002);
TUNE_PARAM(historyMalusLinDepth, 275, 1, 1024, 16, 0.002);
TUNE_PARAM(historyMalusOffset, 59, -1024, 1024, 52, 0.002);

// LMR table values
TUNE_PARAM(lmrDepthValue, 1027, 500, 1500, 50, 0.002);
TUNE_PARAM(lmrMoveValue, 1117, 500, 1500, 50, 0.002);
TUNE_PARAM(lmrA0, 689, 500, 1500, 50, 0.002);
TUNE_PARAM(lmrC0, -206, -1000, 1000, 100, 0.002);
TUNE_PARAM(lmrA1, 814, 500, 1500, 50, 0.002);
TUNE_PARAM(lmrC1, 754, 500, 1500, 50, 0.002);

// LMR additionals
TUNE_PARAM(lmrCieckA, 4418, 2048, 8192, 256, 0.002)
TUNE_PARAM(lmrCieckB, 1472, 500, 2500, 98, 0.002)
TUNE_PARAM(lmrCieckC, 740, 128, 1024, 48, 0.002)
TUNE_PARAM(lmrPV, 1194, 250, 2750, 125, 0.002)
TUNE_PARAM(lmrExpectedDecent, 691, 250, 2750, 125, 0.002)
TUNE_PARAM(lmrQuietHistoryDivisor, 8447, 2000, 14000, 500, 0.002)
TUNE_PARAM(lmrQuietCutNode, 1831, 250, 2750, 125, 0.002);
TUNE_PARAM(lmrQuietTTPV, 972, 250, 2750, 125, 0.002)
TUNE_PARAM(lmrBadNoisyCutNode, 639, 250, 2750, 125, 0.002);
TUNE_PARAM(lmrNoisyHistoryDivisorA, 5149, 2000, 14000, 500, 0.002)
TUNE_PARAM(lmrNoisyHistoryDivisorB, 6685, 2000, 14000, 500, 0.002)

// Deeper / shallower
TUNE_PARAM(doDeeperMargin, 31, 10, 50, 2, 0.002)

// PVS SEE
TUNE_PARAM(pvsSeeThresholdQuiet, -10, -160, -10, 6, 0.002)
TUNE_PARAM(pvsSeeThresholdNoisy, -103, -160, -10, 6, 0.002)
NO_TUNE_PARAM(pvsSeeMaxDepth, 7, 5, 10, 0.5, 0.002)

// LMP values
TUNE_PARAM(lmpA0, 587, 250, 1750, 64, 0.002);
TUNE_PARAM(lmpC0, 1987, 500, 5000, 198, 0.002);
TUNE_PARAM(lmpA1, 986, 250, 1750, 64, 0.002);
TUNE_PARAM(lmpC1, 2890, 500, 5000, 198, 0.002);

// RFP values
TUNE_PARAM(futilityMarginDelta, 95, 45, 145, 4, 0.002);
NO_TUNE_PARAM(RFPDepth, 8, 5, 10, .5, 0.002);

// NMP values
TUNE_PARAM(nmpMarginBias, 180, 100, 300, 8, 0.002);
TUNE_PARAM(nmpDepthMargin, 24, 10, 50, 2, 0.002);
NO_TUNE_PARAM(nmpDepthDivisor, 4, 2, 6, .5, 0.002);
TUNE_PARAM(nmpScoreDivisor, 220, 100, 300, 8, 0.002);
NO_TUNE_PARAM(nmpQ1, 3, 1, 5, .5, 0.002);
NO_TUNE_PARAM(nmpQ2, 3, 0, 5, .5, 0.002);

// Razoring value
TUNE_PARAM(razorQ1, 101, -200, 200, 16, 0.002);
...
TUNE_PARAM(doubleExtensionMargin, 20, 1, 50, 2, 0.002);
NO_TUNE_PARAM(singularSearchDepth, 7, 5, 10, .5, 0.002);

// History Pruning values
TUNE_PARAM(noisyHistoryPruningMultiplier, -1549, -5120, -1024, 205, 0.002);
TUNE_PARAM(noisyHistoryPruningBias, 98, -2048, 2048, 205, 0.002);
TUNE_PARAM(quietHistoryPruningMultiplier, -1582, -5120, -1024, 205, 0.002);
TUNE_PARAM(quietHistoryPruningBias, 59, -2048, 2048, 205, 0.002);
...
TUNE_PARAM(futPruningAdd, 245, 150, 350, 8, 0.002);
NO_TUNE_PARAM(futPruningDepth, 9, 6, 10, 0.5, 0.002);
TUNE_PARAM(qsFutilityMargin, 74, 0, 150, 8, 0.002)

// Move ordering values
TUNE_PARAM(captScoreMvvMultiplier, 14, 8, 32, 1, 0.002)

// Time management values
TUNE_PARAM(timeTmA, 142, 10, 200, 2, 0.002)
TUNE_PARAM(timeTmB, 984, 0, 1000, 13, 0.002)
TUNE_PARAM(timeTmOptimScale, 415, 100, 1000, 11, 0.002)
TUNE_PARAM(nodesTmMax, 1866, 1500, 2500, 13, 0.002)
TUNE_PARAM(nodesTmMul, 1363, 750, 1500, 10, 0.002)

// CorrHist scale values
TUNE_PARAM(MAXCORRHISTUNSCALED, 42, 16, 128, 8, 0.002)
TUNE_PARAM(MAXCORRHISTMILLIUPDATE, 158, 128, 512, 18, 0.002)

// CorrHist correction values
TUNE_PARAM(pawnCorrWeight, 1360, 0, 2048, 102, 0.002)
TUNE_PARAM(nonPawnCorrWeight, 1083, 0, 2048, 102, 0.002)
TUNE_PARAM(T0CorrWeight, 1129, 0, 2048, 102, 0.002)
TUNE_PARAM(T1CorrWeight, 903, 0, 2048, 102, 0.002)
TUNE_PARAM(T2CorrWeight, 955, 0, 2048, 102, 0.002)
TUNE_PARAM(T3CorrWeight, 1204, 0, 2048, 102, 0.002)
TUNE_PARAM(T4CorrWeight, 1239, 0, 2048, 102, 0.002)
TUNE_PARAM(T5CorrWeight, 922, 0, 2048, 102, 0.002)
TUNE_PARAM(T6CorrWeight, 1141, 0, 2048, 102, 0.002)
TUNE_PARAM(T7CorrWeight, 1004, 0, 2048, 102, 0.002)
TUNE_PARAM(T8CorrWeight, 1059, 0, 2048, 102, 0.002)
TUNE_PARAM(T9CorrWeight, 1206, 0, 2048, 102, 0.002)

// CorrHist update values
TUNE_PARAM(pawnCorrUpdate, 1126, 0, 2048, 102, 0.002)
TUNE_PARAM(nonPawnCorrUpdate, 1134, 0, 2048, 102, 0.002)
TUNE_PARAM(T0CorrUpdate, 857, 0, 2048, 102, 0.002)
TUNE_PARAM(T1CorrUpdate, 980, 0, 2048, 102, 0.002)
TUNE_PARAM(T2CorrUpdate, 967, 0, 2048, 102, 0.002)
TUNE_PARAM(T3CorrUpdate, 1051, 0, 2048, 102, 0.002)
TUNE_PARAM(T4CorrUpdate, 1182, 0, 2048, 102, 0.002)
TUNE_PARAM(T5CorrUpdate, 990, 0, 2048, 102, 0.002)
TUNE_PARAM(T6CorrUpdate, 928, 0, 2048, 102, 0.002)
TUNE_PARAM(T7CorrUpdate, 1066, 0, 2048, 102, 0.002)
TUNE_PARAM(T8CorrUpdate, 1223, 0, 2048, 102, 0.002)
TUNE_PARAM(T9CorrUpdate, 996, 0, 2048, 102, 0.002)
