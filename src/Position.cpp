...
inline void Position::addQuiet(MoveList *ml, ScoredMove move, Square source, Square target, Move killer1, Move killer2, Move counterMove, const S32 *ply1contHist, const S32 *ply2contHist, const S32 gamePhase) {
    ...
    ml->moves[ml->count++] = ((
        (S64)(phasedHistoryTable[0][indexSideFromTo(side, source, target)] * gamePhase / 24)
        + (S64)(phasedHistoryTable[1][indexSideFromTo(side, source, target)] * (24 - gamePhase) / 24)
...
void Position::generateMoves(MoveList& moveList, SStack* ss) {
    ...
    const S32 gamePhase = std::min(24, popcount(bitboards[N] | bitboards[n] | bitboards[B] | bitboards[b]) + 2 * popcount(bitboards[R] | bitboards[r]) + 4 * popcount(bitboards[Q] | bitboards[q]));
    ...
    if (side == WHITE) {
        ...
        while (dxPawnCaptures) {
            ...
            if (to <= h8) {
                // We have a promotion
                addPromoCapture(&moveList, encodeMove(to + 7, to, P, captured, Q, false, false, false, (queenCheckers & squareBB(to)) > 0) , P, captured, Q);
                addPromoCapture(&moveList, encodeMove(to + 7, to, P, captured, R, false, false, false, (rookCheckers & squareBB(to)) > 0) , P, captured, R);
                addPromoCapture(&moveList, encodeMove(to + 7, to, P, captured, B, false, false, false, (bishopCheckers & squareBB(to)) > 0) , P, captured, B);
                addPromoCapture(&moveList, encodeMove(to + 7, to, P, captured, N, false, false, false, (knightCheckers & squareBB(to)) > 0) , P, captured, N);
            }
            else
        ...
        while (sxPawnCaptures) {
            ...
            if (to <= h8) {
                // We have a promotion
                addPromoCapture(&moveList, encodeMove(to + 9, to, P, captured, Q, false, false, false, (queenCheckers & squareBB(to)) > 0), P, captured, Q);
                addPromoCapture(&moveList, encodeMove(to + 9, to, P, captured, R, false, false, false, (rookCheckers & squareBB(to)) > 0), P, captured, R);
                addPromoCapture(&moveList, encodeMove(to + 9, to, P, captured, B, false, false, false, (bishopCheckers & squareBB(to)) > 0), P, captured, B);
                addPromoCapture(&moveList, encodeMove(to + 9, to, P, captured, N, false, false, false, (knightCheckers & squareBB(to)) > 0), P, captured, N);
            }
            else
    ...
    while (knights) {
        ...
        while (quiets) {
            ...
            bool isCheck = (knightCheckers & squareBB(to)) > 0;
            addQuiet(&moveList, encodeMove(from, to, N + 6 * side, NOPIECE, NOPIECE, false, false, false, isCheck), from, to, killer1, killer2, counterMove, ply1contHist, ply2contHist, gamePhase);
        }
    }
    ...
    while (bishops) {
        ...
        while (quiets) {
            Square to = popLsb(quiets);
            bool isCheck = (bishopCheckers & squareBB(to)) > 0;
            addQuiet(&moveList, encodeMove(from, to, B + 6 * side, NOPIECE, NOPIECE, false, false, false, isCheck), from, to, killer1, killer2, counterMove, ply1contHist, ply2contHist, gamePhase);
        }
    }
    ...
    while (rooks) {
        ...
        while (quiets) {
            Square to = popLsb(quiets);
            bool isCheck = (rookCheckers & squareBB(to)) > 0;
            addQuiet(&moveList, encodeMove(from, to, R + 6 * side, NOPIECE, NOPIECE, false, false, false, isCheck), from, to, killer1, killer2, counterMove, ply1contHist, ply2contHist, gamePhase);
        }
    }
    ...
    while (queens) {
        ...
        while (quiets) {
            Square to = popLsb(quiets);
            bool isCheck = (queenCheckers & squareBB(to)) > 0;
            addQuiet(&moveList, encodeMove(from, to, Q + 6 * side, NOPIECE, NOPIECE, false, false, false, isCheck), from, to, killer1, killer2, counterMove, ply1contHist, ply2contHist, gamePhase);
        }
    }
    ...
    if (side == WHITE) {
        ...
        while (pawnPushes) {
            ...
            if (to <= h8) {
                ...
                addQuiet(&moveList, encodeMove(to + 8, to, P, NOPIECE, NOPIECE, false, false, false, (pawnCheckers & squareBB(to)) > 0), to + 8, to, killer1, killer2, counterMove, ply1contHist, ply2contHist, gamePhase);
        }
        ...
        while (pawnDoublePushes) {
            ...
            addQuiet(&moveList, encodeMove(to + 16, to, P, NOPIECE, NOPIECE, true, false, false, (pawnCheckers & squareBB(to)) > 0), to + 16, to, killer1, killer2, counterMove, ply1contHist, ply2contHist, gamePhase);
        }
    }
    else {
        BitBoard pawnPushes = ourPawns << 8;
    ...
    while (kMoves) {
        ...
        addQuiet(&moveList, encodeMove(king, to, K + 6 * side, NOPIECE, NOPIECE, false, false, false, false), king, to, killer1, killer2, counterMove, ply1contHist, ply2contHist, gamePhase);
    }
    // We will generate the castling moves
    if (side == WHITE) {
        if (castle & CastleRights::WK && !(occupancy & wKCastleMask)) {
            addQuiet(&moveList, encodeMove(e1, g1, K, NOPIECE, NOPIECE, false, false, true, false), e1, g1, killer1, killer2, counterMove, ply1contHist, ply2contHist, gamePhase);
        }
        if (castle & CastleRights::WQ && !(occupancy & wQCastleMask)) {
            addQuiet(&moveList, encodeMove(e1, c1, K, NOPIECE, NOPIECE, false, false, true, false), e1, c1, killer1, killer2, counterMove, ply1contHist, ply2contHist, gamePhase);
        }
    }
    else {
        if (castle & CastleRights::BK && !(occupancy & bKCastleMask)) {
            addQuiet(&moveList, encodeMove(e8, g8, k, NOPIECE, NOPIECE, false, false, true, false), e8, g8, killer1, killer2, counterMove, ply1contHist, ply2contHist, gamePhase);
        }
        if (castle & CastleRights::BQ && !(occupancy & bQCastleMask)) {
            addQuiet(&moveList, encodeMove(e8, c8, k, NOPIECE, NOPIECE, false, false, true, false), e8, c8, killer1, killer2, counterMove, ply1contHist, ply2contHist, gamePhase);
        }
    }
}
