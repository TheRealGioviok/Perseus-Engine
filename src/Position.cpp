...
bool Position::givesCheck(Move move){
    ...
    bool side = movePiece(move) >= p;
    Piece pt = movePiece(move) - 6*side;
    if (pt < K && ptCheckers[pt] & squareBB(to)) return true;

    Square otherKing = side ? lsb(bitboards[K]) : lsb(bitboards[k]);

    if ((blockersForKing[!side] & squareBB(from)) && !aligned(from, to, otherKing)) return true;

    switch (isPromotion(move) + 2*isEnPassant(move) + 4*isCastling(move)){
        case 0: return false;
        case 1: {
            switch (movePromotion(move)){
                case N: case n:
                    return knightAttacks[to] & squareBB(otherKing);
                case B: case b:
                    return getBishopAttack(to, occupancies[BOTH] ^ squareBB(from)) & squareBB(otherKing);
                case R: case r:
                    return getRookAttack(to, occupancies[BOTH] ^ squareBB(from)) & squareBB(otherKing);
                case Q: case q:
                    return getQueenAttack(to, occupancies[BOTH] ^ squareBB(from)) & squareBB(otherKing);
            }
        }
        
        case 2: {
            BitBoard modPieces = occupancies[BOTH] ^ squareBB(to) ^ squareBB(enPassant + (side ? -8 : +8));
            BitBoard* ourPieces = side ? bitboards + 6 : bitboards;
            return (getBishopAttack(otherKing, modPieces) & (ourPieces[B] | ourPieces[Q])) |
                   (getRookAttack(otherKing, modPieces) & (ourPieces[R] | ourPieces[Q]));
        }
        default: {
            switch (to) {
                case c1: return getRookAttack(d1, occupancies[BOTH] ^ squareBB(e1) ^ squareBB(c1) ^ squareBB(a1) ^ squareBB(d1)) & squareBB(otherKing);
                case g1: return getRookAttack(f1, occupancies[BOTH] ^ squareBB(e1) ^ squareBB(g1) ^ squareBB(h1) ^ squareBB(f1)) & squareBB(otherKing);
                case c8: return getRookAttack(d8, occupancies[BOTH] ^ squareBB(e8) ^ squareBB(c8) ^ squareBB(a8) ^ squareBB(d8)) & squareBB(otherKing);
                case g8: return getRookAttack(f8, occupancies[BOTH] ^ squareBB(e8) ^ squareBB(g8) ^ squareBB(h8) ^ squareBB(f8)) & squareBB(otherKing);
            }
        }
    }
