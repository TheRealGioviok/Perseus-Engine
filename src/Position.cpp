...
/** 
 * @brief The Position::generateMajorHashKey function generates the hash key of the minors + kings from scratch, for a given side.
 * @note This function is called by the constructors. Otherwise the hash gets incrementally updated.
 */
...
/** 
 * @brief The Position::generateMajorHashKey function generates the hash key of the majors + kings from scratch, for a given side.
 * @note This function is called by the constructors. Otherwise the hash gets incrementally updated.
 */
...
static inline void updateHashKeys(Position& pos, const Piece piece, const Square square){
    bool side = (piece >= p);
    const Piece pt = piece - 6 * side;
    const HashKey hash = pieceKeysTable[piece][square];
    pos.hashKey ^= hash;
    switch (pt){
        case P:
            pos.pawnHashKey ^= hash;
            break;
        case N: case B:
            pos.nonPawnKeys[side] ^= hash;
            pos.minorKey ^= hash;
            break;
        case R: case Q:
            pos.nonPawnKeys[side] ^= hash;
...
static inline void removePiece(Position& pos, const Piece piece, const Square square){
    ...
    // Update the hash key
    updateHashKeys(pos, piece, square);
