...
void Position::generateMoves(MoveList& moveList, SStack* ss) {
    ...
    if (side == WHITE) {
        ...
        while (dxPawnCaptures) {
            ...
            if (to <= h8) {
                // We have a promotion
                addPromoCapture<Q>(&moveList, encodeMove(to + 7, to, P, captured, Q, false, false, false, false) , P, captured);
                addPromoCapture<R>(&moveList, encodeMove(to + 7, to, P, captured, R, false, false, false, false) , P, captured);
                addPromoCapture<B>(&moveList, encodeMove(to + 7, to, P, captured, B, false, false, false, false) , P, captured);
                addPromoCapture<N>(&moveList, encodeMove(to + 7, to, P, captured, N, false, false, false, false) , P, captured);
            }
            else
                addCapture(&moveList, encodeMove(to + 7, to, P, captured, NOPIECE, false, false, false, false), P, captured);
        }
        // Left captures
        while (sxPawnCaptures) {
            ...
            if (to <= h8) {
                // We have a promotion
                addPromoCapture<Q>(&moveList, encodeMove(to + 9, to, P, captured, Q, false, false, false, false), P, captured);
                addPromoCapture<R>(&moveList, encodeMove(to + 9, to, P, captured, R, false, false, false, false), P, captured);
                addPromoCapture<B>(&moveList, encodeMove(to + 9, to, P, captured, B, false, false, false, false), P, captured);
                addPromoCapture<N>(&moveList, encodeMove(to + 9, to, P, captured, N, false, false, false, false), P, captured);
            }
            else
                addCapture(&moveList, encodeMove(to + 9, to, P, captured, NOPIECE, false, false, false, false), P, captured);
        }
        ...
        if (enPassant != noSquare) {
            ...
            while (epPawns) {
                ...
                addEp(&moveList, encodeMove(from, to, P, p, NOPIECE, false, true, false, false));
            }
        }
    }
    else {
    ...
    while (knights) {
        ...
        while (attacks) {
            ...
            addCapture(&moveList, encodeMove(from, to, N + 6 * side, captured, NOPIECE, false, false, false, false), N + 6 * side, captured);
        }
        while (quiets) {
    ...
    while (bishops) {
        ...
        while (attacks) {
            ...
            addCapture(&moveList, encodeMove(from, to, B + 6 * side, captured, NOPIECE, false, false, false, false), B + 6 * side, captured);
        }
        while (quiets) {
            Square to = popLsb(quiets);
            bool isCheck = (bishopCheckers & squareBB(to)) > 0;
    ...
    while (rooks) {
        ...
        while (attacks) {
            ...
            addCapture(&moveList, encodeMove(from, to, R + 6 * side, captured, NOPIECE, false, false, false, false), R + 6 * side, captured);
        }
        while (quiets) {
            Square to = popLsb(quiets);
            bool isCheck = (rookCheckers & squareBB(to)) > 0;
    ...
    while (queens) {
        ...
        while (attacks) {
            ...
            addCapture(&moveList, encodeMove(from, to, Q + 6 * side, captured, NOPIECE, false, false, false, false), Q + 6 * side, captured);
        }
        while (quiets) {
            Square to = popLsb(quiets);
            bool isCheck = (queenCheckers & squareBB(to)) > 0;
    ...
    if (side == WHITE) {
        ...
        while (pawnPushes) {
            ...
            if (to <= h8) {
                ...
                addPromotion<Q>(&moveList, encodeMove(to + 8, to, P, NOPIECE, Q, false, false, false, false));
                addPromotion<R>(&moveList, encodeMove(to + 8, to, P, NOPIECE, R, false, false, false, false));
                addPromotion<B>(&moveList, encodeMove(to + 8, to, P, NOPIECE, B, false, false, false, false));
                addPromotion<N>(&moveList, encodeMove(to + 8, to, P, NOPIECE, N, false, false, false, false));
            }
            else
                addQuiet(&moveList, encodeMove(to + 8, to, P, NOPIECE, NOPIECE, false, false, false, false), to + 8, to, killer1, killer2, counterMove, ply1contHist, ply2contHist);
        }
        ...
        while (pawnDoublePushes) {
            ...
            addQuiet(&moveList, encodeMove(to + 16, to, P, NOPIECE, NOPIECE, true, false, false, false), to + 16, to, killer1, killer2, counterMove, ply1contHist, ply2contHist);
        }
    }
    else {
        BitBoard pawnPushes = ourPawns << 8;
...
void Position::generateUnsortedMoves(MoveList& moveList) {
    ...
    if (side == WHITE) {
        ...
        while (dxPawnCaptures) {
            ...
            if (to <= h8) {
                // We have a promotion
                addUnsorted(&moveList, encodeMove(to + 7, to, P, captured, Q, false, false, false, false));
                addUnsorted(&moveList, encodeMove(to + 7, to, P, captured, R, false, false, false, false));
                addUnsorted(&moveList, encodeMove(to + 7, to, P, captured, B, false, false, false, false));
                addUnsorted(&moveList, encodeMove(to + 7, to, P, captured, N, false, false, false, false));
            }
            else
                addUnsorted(&moveList, encodeMove(to + 7, to, P, captured, NOPIECE, false, false, false, false));
        }
        // Left captures
        while (sxPawnCaptures) {
            Square to = popLsb(sxPawnCaptures);
            ...
            if (to <= h8) {
                // We have a promotion
                addUnsorted(&moveList, encodeMove(to + 9, to, P, captured, Q, false, false, false, false));
                addUnsorted(&moveList, encodeMove(to + 9, to, P, captured, R, false, false, false, false));
                addUnsorted(&moveList, encodeMove(to + 9, to, P, captured, B, false, false, false, false));
                addUnsorted(&moveList, encodeMove(to + 9, to, P, captured, N, false, false, false, false));
            }
            else
                addUnsorted(&moveList, encodeMove(to + 9, to, P, captured, NOPIECE, false, false, false, false));
        }
        // En passant capture (if there is one)
        if (enPassant != noSquare) {
            Square to = enPassant;
            ...
            while (epPawns) {
                Square from = popLsb(epPawns);
                addUnsorted(&moveList, encodeMove(from, to, P, p, NOPIECE, false, true, false, false));
            }
        }
    }
    else {
    ...
    if (side == WHITE) {
        ...
        while (pawnPushes) {
            ...
            if (to <= h8) {
                // We will generate the promotion moves
                addUnsorted(&moveList, encodeMove(to + 8, to, P, NOPIECE, Q, false, false, false, false));
                addUnsorted(&moveList, encodeMove(to + 8, to, P, NOPIECE, R, false, false, false, false));
                addUnsorted(&moveList, encodeMove(to + 8, to, P, NOPIECE, B, false, false, false, false));
                addUnsorted(&moveList, encodeMove(to + 8, to, P, NOPIECE, N, false, false, false, false));
            }
            else
                addUnsorted(&moveList, encodeMove(to + 8, to, P, NOPIECE, NOPIECE, false, false, false, false));
        }
        ...
        while (pawnDoublePushes) {
            Square to = popLsb(pawnDoublePushes);
            addUnsorted(&moveList, encodeMove(to + 16, to, P, NOPIECE, NOPIECE, true, false, false, false));
        }
    }
    else {
        BitBoard pawnPushes = ourPawns << 8;
