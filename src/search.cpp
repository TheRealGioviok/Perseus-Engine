...
#define UCILATENCYMS 20
    ...
    for (currSearch = 2; (currSearch <= depth) && currSearch >= 2 && !stopped; currSearch++)
    {
        ...
        if (currSearch >= 6){
            // Percentage ( 0.699385 ) calculated with bench @24
            nodesTmScale = 2.0 - ((double)nodesPerMoveTable[indexFromTo(moveSource(bestMove), moveTarget(bestMove))] / (double)nodes) * 1.429827634;    
        }
