            ...
            if (moveSearched > PVNode * 3 && depth >= 3 && (isQuiet || !ttPv))
            {
                ...
                if (isQuiet){
                    ...
                    if (currMoveScore < QUIETSCORE) { 
                        ...
                        granularR -= std::clamp((currMoveScore - BADNOISYMOVE) * RESOLUTION, -16384000LL, 16384000LL) / lmrNoisyHistoryDivisorA();
                    }
                    else {
                        granularR -= std::clamp((currMoveScore - GOODNOISYMOVE - BADNOISYMOVE) * RESOLUTION, -16384000LL, 16384000LL) / lmrNoisyHistoryDivisorB();
                    }
                }
