...
constexpr PScore R_SUPPORTEDPHALANX = S(4, 0);
constexpr PScore R_ADVANCABLEPHALANX = S(3, 1);
constexpr PScore passedRankBonus[2][7] = {
    {S(0, 0), S(14, -65), S(-3, -43), S(2, 11), S(20, 66), S(44, 157), S(118, 232)}, 
    {S(0, 0), S(0, 0), S(11, -57), S(12, -2), S(55, 66), S(155, 181), S(400, 254)}
};
constexpr PScore PASSEDPATHBONUS = S(-6, 23);
constexpr PScore candidateRankBonus[2][7] = {
    {S(0, 0), S(-35, 5), S(-24, 23), S(-11, 61), S(2, 127), S(28, 106), S(0, 0)}, 
    {S(0, 0), S(0, 0), S(-6, 33), S(2, 68), S(42, 144), S(119, 168), S(0, 0)}
};
constexpr PScore INNERSHELTER = S(3, -18);
constexpr PScore OUTERSHELTER = S(12, -12);
...
constexpr PScore BISHOPPAWNS = S(0, -6);
...
constexpr PScore PAWNTHREAT[2][5] = {
    {S(11, -67), S(31, 77), S(27, 114), S(26, 40), S(8, 32)}, 
    {S(5, -27), S(12, 128), S(13, 163), S(23, 92), S(15, 92)}
};
constexpr PScore KNIGHTTHREAT[2][5] = {
    {S(7, 19), S(0, -31), S(25, 18), S(59, -12), S(41, 14)}, 
    {S(-7, 6), S(1, 17), S(26, 71), S(54, 59), S(44, 45)}
};
constexpr PScore BISHOPTHREAT[2][5] = {
    {S(11, 17), S(32, 21), S(-12, -40), S(57, -9), S(55, 21)}, 
    {S(0, 7), S(33, 68), S(-8, 11), S(51, 66), S(50, 68)}
};
constexpr PScore ROOKTHREAT[2][5] = {
    {S(3, 18), S(12, 48), S(22, 36), S(-8, -61), S(62, -18)}, 
    {S(-10, 5), S(2, 24), S(7, 22), S(-1, -5), S(46, 38)}
};
constexpr PScore QUEENTHREAT[2][5] = {
    {S(12, -2), S(27, -4), S(27, 23), S(-2, -22), S(4, -140)}, 
    {S(-2, 9), S(7, -1), S(5, 14), S(0, 5), S(9, -120)}
};
...
constexpr PScore PAWNATTACKINNERRING = S(62, -125);
constexpr PScore KNIGHTATTACKINNERRING = S(82, -18);
constexpr PScore BISHOPATTACKINNERRING = S(114, 54);
constexpr PScore ROOKATTACKINNERRING = S(118, 46);
constexpr PScore QUEENATTACKINNERRING = S(76, -77);
constexpr PScore PAWNATTACKOUTERRING = S(72, -38);
constexpr PScore KNIGHTATTACKOUTERRING = S(83, 48);
constexpr PScore BISHOPATTACKOUTERRING = S(82, 47);
constexpr PScore ROOKATTACKOUTERRING = S(29, 13);
constexpr PScore QUEENATTACKOUTERRING = S(55, 99);
constexpr PScore NOQUEENDANGER = S(-881, -676);
constexpr PScore PINNEDSHELTERDANGER = S(104, -67);
constexpr PScore FLANKATTACKS[2] = {S(31, -39), S(42, -51), };
constexpr PScore FLANKDEFENSES[2] = {S(-34, 17), S(-9, 13), };
constexpr PScore SAFECHECK[4] = {S(317, -54), S(89, 39), S(223, 101), S(226, 28), };
constexpr PScore ALLCHECKS[4] = {S(80, 17), S(22, 94), S(68, 28), S(12, 32), };
constexpr PScore SAFETYINNERSHELTER = S(-45, -103);
constexpr PScore SAFETYOUTERSHELTER = S(5, -182);
constexpr PScore INNERWEAKNESS = S(104, -61);
constexpr PScore OUTERWEAKNESS = S(12, 15);
constexpr PScore KSTEMPO = S(26, 23);

constexpr Score COMPLEXITYPASSERS = 464;
constexpr Score COMPLEXITYPAWNS = 1138;
constexpr Score COMPLEXITYBLOCKEDPAIRS = -384;
constexpr Score COMPLEXITYPAWNTENSION = -731;
constexpr Score COMPLEXITYOUTFLANKING = -46;
constexpr Score COMPLEXITYINFILTRATION = -772;
constexpr Score COMPLEXITYPAWNBOTHFLANKS = 10206;
constexpr Score COMPLEXITYPAWNENDING = 13228;
constexpr Score COMPLEXITYALMOSTUNWINNABLE = -3225;
constexpr Score COMPLEXITYBIAS = -21466;
...
Score pestoEval(Position *pos){
    ...
    const BitBoard outpostSquares[2] = {
        (ranks(2) | ranks(3) | ranks(4)) & pawnAttackedSquares[WHITE] & ~(makePawnAttacks<BLACK>(pawnSpan[BLACK])) & notFile(0) & notFile(7),
        (ranks(5) | ranks(4) | ranks(3)) & pawnAttackedSquares[BLACK] & ~(makePawnAttacks<WHITE>(pawnSpan[WHITE])) & notFile(0) & notFile(7)
    };
